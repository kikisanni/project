...
export default function Summary({ userAnswers, user }: SummaryProps) {
  ...
  const updateQuizResults = async () => {
    try {
      ...
      if (existingResults.completed !== 5) {
        updatedPoints += points;
        updatedHearts -= wrongAnswer;
        existingResults.completed = 5; // Update total completed to 4
      } else {
        // Calculate new total points
        const prevTotalPoints = existingResults.categoryTotalPoints[3]; //total points from category 1 easy
      ...
      updatedPoints = Math.min(updatedPoints, 20);

      console.log("updated", updatedPoints);
      let totalPoints = Math.max(updatedPoints, existingResults.totalPoints);

      const totalXP = Math.round((totalPoints / 20) * 100);
      console.log(totalPoints);
      console.log(totalXP);

      // Append the new points to the categoryTotalPoints array
      if (
        existingResults.categoryTotalPoints.length < 5 ||
        existingResults.categoryTotalPoints[4] === undefined
      ) {
        existingResults.categoryTotalPoints.push(totalPoints);
      } else {
        existingResults.categoryTotalPoints[4] = totalPoints;
      }
