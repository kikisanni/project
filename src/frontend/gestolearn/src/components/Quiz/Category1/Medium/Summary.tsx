...
export default function Summary({ userAnswers, user }: SummaryProps) {
  ...
  const updateQuizResults = async () => {
    try {
      ...
      if (existingResults.completed !== 2) {
        updatedPoints += points;
        updatedHearts -= wrongAnswer;
        existingResults.completed = 2; // Update total completed to 2
      } else {
        // Calculate new total points
        const prevTotalPoints = existingResults.categoryTotalPoints[0]; //total points from category 1 easy
        ...
        updatedHearts =
          existingResults.hearts > 3 ? existingResults.hearts : 3 - wrongAnswer;
      }

      updatedPoints = Math.min(updatedPoints, 8);

      console.log("updated", updatedPoints);
      let totalPoints = Math.max(updatedPoints, existingResults.totalPoints);

      const totalXP = Math.round((totalPoints / 8) * 100);
      console.log(totalPoints);
      console.log(totalXP);

      // Append the new points to the categoryTotalPoints array
      if (
        existingResults.categoryTotalPoints.length < 2 ||
        existingResults.categoryTotalPoints[1] === undefined
      ) {
        existingResults.categoryTotalPoints.push(totalPoints);
      } else {
        existingResults.categoryTotalPoints[1] = totalPoints;
      }
  ...
  const restartQuiz = () => {
    ...
    // Then call function to update restarted scores
  };
    ...
    <div className="container mx-auto w-3/5 mt-8 rounded-lg mb-10 bg-purple-400 p-8 shadow-2xl shadow-gray-600 flex justify-center items-center font-serif">
      <div className="grid">
        ...
        <h2 className="font-semibold text-center">
          ...
          maximum achievable hearts to 3.
        </h2>
