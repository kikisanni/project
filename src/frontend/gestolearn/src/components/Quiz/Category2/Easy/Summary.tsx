...
export default function Summary({ userAnswers, user }: SummaryProps) {
  ...
  const updateQuizResults = async () => {
    try {
      ...
      if (existingResults.completed !== 3) {
        updatedPoints += points;
        updatedHearts -= wrongAnswer;
        existingResults.completed = 3; // Update total completed to 3
      } else {
        // Calculate new total points
        const prevTotalPoints = existingResults.categoryTotalPoints[1]; //total points from category 1 medium
      ...
      updatedPoints = Math.min(updatedPoints, 12);

      console.log("updated", updatedPoints);
      let totalPoints = Math.max(updatedPoints, existingResults.totalPoints);

      const totalXP = Math.round((totalPoints / 12) * 100);
      console.log(totalPoints);
      console.log(totalXP);

      // Append the new points to the categoryTotalPoints array
      if (
        existingResults.categoryTotalPoints.length < 3 ||
        existingResults.categoryTotalPoints[2] === undefined
      ) {
        existingResults.categoryTotalPoints.push(totalPoints);
      } else {
        existingResults.categoryTotalPoints[2] = totalPoints;
      }
