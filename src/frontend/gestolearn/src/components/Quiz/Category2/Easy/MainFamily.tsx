...
function MainFamily({ user }: MainFamilyProps) {
  ...
  const modalContent = () => {
    if (existingResults?.hearts === 0 && existingResults?.completed !== 3 || existingResults?.hearts === 1 && existingResults?.completed !== 3) {
      return (
        <div>
      ...
      window.location.href = "/quiz-2-easy";
    }
  };
