...
function MediumFamily({ user }: MediumFamilyProps) {
  ...
  const modalContent = () => {
    if (
      (existingResults?.hearts === 0 && existingResults?.completed !== 4) ||
      (existingResults?.hearts === 1 && existingResults?.completed !== 4)
    ) {
      return (
        <div>
          <h2 className="text-xl font-semibold mb-2">Out of Hearts!</h2>
          <p className="mb-4">
            You need hearts to start the quiz. Go back to the previous category
            to gather more hearts!
          </p>
        </div>
      );
    } else if (
      ...
      window.location.href = "/quiz-2-medium";
    }
  };
