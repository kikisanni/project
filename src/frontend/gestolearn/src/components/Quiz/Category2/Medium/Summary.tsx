...
export default function Summary({ userAnswers, user }: SummaryProps) {
  ...
  const updateQuizResults = async () => {
    try {
      ...
      if (existingResults.completed !== 4) {
        updatedPoints += points;
        updatedHearts -= wrongAnswer;
        existingResults.completed = 4; // Update total completed to 4
      } else {
        // Calculate new total points
        const prevTotalPoints = existingResults.categoryTotalPoints[2]; //total points from category 1 easy
      ...
      updatedPoints = Math.min(updatedPoints, 16);

      console.log("updated", updatedPoints);
      let totalPoints = Math.max(updatedPoints, existingResults.totalPoints);

      const totalXP = Math.round((totalPoints / 16) * 100);
      console.log(totalPoints);
      console.log(totalXP);

      // Append the new points to the categoryTotalPoints array
      if (
        existingResults.categoryTotalPoints.length < 4 ||
        existingResults.categoryTotalPoints[3] === undefined
      ) {
        existingResults.categoryTotalPoints.push(totalPoints);
      } else {
        existingResults.categoryTotalPoints[3] = totalPoints;
      }
